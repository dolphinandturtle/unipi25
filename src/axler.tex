...
\begin{multicols}{3}
  ...
  The sum of a list of subspaces is the smallest subspace containing them individually.
  ...
  A sum between a pair of subspaces is direct if and only if only the additive identity is in their intersection.

  \mysection{span and linear independence}
  \mydefinition{Linear combination}
  A sum on a list of vectors each individually scaled by some value.
  ...
  \mydefinition{Spanning}
  Whose span is equal to.
  
  \mydefinition{Finite-dimensional vector space}
  A vector space spanned by some list of vectors.
  
  \mydefinition{Polynomial with scalar coefficients}
  A function mapping field elements into themselves by summing coefficients multiplied by an element raised to the power of the coefficient's index.
  
  \mydefinition{Degree of a polynomial}
  The greatest index of a non-zero coefficient.
  
  \mydefinition{Infinite-dimensional vector space}
  A vector space spanned by no list of vectors.
  
  \mydefinition{Linearly independent}
  A list of vectors whose null linear combination implies all coefficients to be zero.
  
  \mydefinition{Linearly dependent}
  A list of vectors whose null linear combination contains a coefficient that isn't zero.
  ...
  The dimension of their sum is the sum of their dimensions minus the dimension of their intersection.
